        ...
        <div class="centrarHome">
            <b-collapse id="nav-collapse" is-nav>
                <b-navbar-nav class="items">
                    ...
                    <b-button v-if="!this.user.id" @click="$router.push({ name: 'Registro' })" pill class="boton">Registrarme</b-button>
                    <b-button v-if="!this.user.id" @click="redirectoToLogin()" pill class="boton2">Ingresar</b-button>
                    <b-button v-if="!!this.user.id" @click="$router.push({ path: '/misDatos/' + user.id })" class="bg-transparent border-0">
                        <b-img v-if="this.user.foto != '' && this.user.foto" rounded="circle" :src="this.user.foto" width="60" class="fotoUsuario"></b-img>
                    </b-button>
                    <b-button v-if="!!this.user.id" @click="logOut()" class="bg-transparent border-0">
                ...
                <b-collapse id="nav-collapse" is-nav>
                    <b-navbar-nav class="items">
                        ...
                        <b-nav-item :to="'/misdatos/' + this.user.id">
                            <b-img :src="user.foto" alt="" id="foto" fluid class="botonImagenHeader fotoUsuario">
                            </b-img>
                            <label id="usuario">{{ user.nombre + " " + user.apellido }}</label>
                        </b-nav-item>
                        <Carrito ref="carrito" :productos="[{id:1, descripcion:'Pizza Muzzarella Grande', cantidad:2, precio:1500.12 },{id:2,descripcion:'Pizza Muzzarella Grande', cantidad:2, precio:1500 }]" />
            ...
            <div v-else>
                <b-collapse id="nav-collapse" is-nav>
                    <b-navbar-nav class="itemsEmpleado">
                        <b-nav-item :to="'/misdatos/' + this.user.id">
                            <b-img v-if="this.user.foto != '' && this.user.foto" :src="this.user.foto" id="foto" fluid class="botonImagenHeader fotoUsuario">
                            </b-img>
                            <b-img v-else src="http://localhost:9001/images/sistema/userDefaultChico.png" alt="" id="foto" fluid class="botonImagenHeader">
                            </b-img>
                            <label v-if="this.user.nombre && this.user.apellido" id="usuario">{{ this.user.nombre + " " + this.user.apellido }}
                            </label>
                        </b-nav-item>
                        <b-nav-item class="menuLateral" v-for="boton in botones" :key="boton[0]" v-bind:href="boton[3]">
                            <b-img v-bind:src="'http://localhost:9001/images/sistema/' + boton[2]" fluid class="iconosMenu"></b-img>
        ...
        redirectoToLogin() {
            ...
            this.$bvModal
                ...
                .then((value) => {
                    ...
                    else if (value == false) this.$router.push({
                        name: "Ingreso"
                    });
                })
                .catch((e) => {
