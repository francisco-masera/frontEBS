		...
		<div id="home3">
			...
			<div class="cuadro" style="background-color: #1f91b6">
				<div class="cuadroText">
					...
					Comenzamos este emprendimiento en el año 2017, con la esperanza de
					poder crecer como profesionales y generar una gran amistad entre
					nosotros. Nos enfocamos en un producto de calidad, intentando brindar
					lo mejor de nosotros para que nuestros clientes queden satisfechos.
				...
				<div class="cuadroText">
					...
					<div class="enlaces">
						...
						<a :href="'mailto:info@elbuensabor.com'" target="_blank"
							><img
								:src="require('@/assets/images/correo.svg')"
							/>info@elbuensabor.com</a
						>
						<br />
						<a :href="'tel:+54 261 156789873'"
							><img :src="require('@/assets/images/telefono.svg')" />+54 261
							156789873</a
						>
						<br />
						<a :href="'https://maps.google.com/?q=-32.890848,-68.839388'"
							><img :src="require('@/assets/images//direccion.svg')" />San
							Martín 1087, Mendoza</a
						>
					</div>
				</div>
			</div>
			<div class="cuadro">
				<iframe
					:src="'https://maps.google.com/maps?q=-32.890848,-68.839388&z=15&output=embed'"
					width="100%"
					height="100%"
					frameborder="0"
					style="border: 0"
		...
		methods: {
			getAllProductos() {
				this.service
					...
					.then((r) => r.forEach((d) => this.allProductos.push(d)))
					.then(
						this.service.getAll("insumoVenta/masVendidos").then((r) => {
							r.forEach((d) => this.allProductos.push(d));
							this.loading = false;
						})
					)
					.catch((this.loading = false));
				this.venta = this.allProductos;
			},
			filtrar() {
				...
				this.timeout = setTimeout(() => {
					this.filtrados = [];
					if (this.buscar != "") {
						this.filtrados = this.allProductos.filter((item) => {
							if (item.type == "ArticuloManufacturado") {
								return item.denominacion
									.toLowerCase()
									.includes(this.buscar.toLowerCase());
							} else {
								return item.insumo.denominacion
									.toLowerCase()
									.includes(this.buscar.toLowerCase());
							}
						});
						this.venta = this.filtrados;
					} else {
						this.venta = this.allProductos;
					}
				}, 800);
